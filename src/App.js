...
export default function App() {
  ...
  const updateLeaderboardProfile = async (db, userId, data) => {
      ...
      try {
          ...
          console.error("Error updating leaderboard profile:", error);
      }
  };

  const getDefaultUserData = () => ({
    name: "Alex", studyStreak: 0, points: 0, level: 1, overallProgress: 0, domains: [ { id: 1, name: "Coaching Structure", progress: 0, weight: 15 }, { id: 2, name: "Coaching Process", progress: 0, weight: 48 }, { id: 3, name: "Health & Wellness", progress: 0, weight: 23 }, { id: 4, name: "Ethics, Legal & Professional", progress: 0, weight: 14 }, ], todaysPlan: [ { task: "Take your first practice quiz!", time: 15, completed: false }, { task: "Explore the 'Coaching Process' domain", time: 20, completed: false }, ], achievements: [ { id: 'streak10', unlocked: false }, { id: 'domainMasterEthics', unlocked: false }, { id: 'comebackKid', unlocked: false }, { id: 'knowledgeSeeker', unlocked: false }, { id: 'quizWhiz', unlocked: false } ],
  });
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(docRef, async (snapshot) => {
      if (snapshot.exists()) { 
          ...
          if (userData && data.level > userData.level && appSettings.showNotifications) {
              setLevelUpInfo({ level: data.level });
          }
          setUserData(data);
          updateLeaderboardProfile(db, userId, data);
      } else {
        ...
        try { 
            ...
            updateLeaderboardProfile(db, userId, defaultUserData);
        } catch (error) { console.error("Error creating user profile:", error); }
      }
    });
    return () => unsubscribe();
  }, [db, userId, userData, appSettings.showNotifications]);
  ...
  const handleAnswer = async (questionId, isCorrect) => {
      ...
      try {
          ...
          if (docSnap.exists()) {
              currentStrength = docSnap.data().strength;
          }
  ...
  const handleQuizCompletion = async (score, totalQuestions, lastAnswer = null) => {
      if (lastAnswer) {
          handleAnswer(lastAnswer.questionId, lastAnswer.isCorrect);
          return;
      }

      if (!db || !userId || !userData) return;
      const pointsEarned = score * 10;
      const newTotalPoints = userData.points + pointsEarned;
      const newProgress = Math.min(100, userData.overallProgress + Math.round((score / totalQuestions) * 2));
      const newAchievements = userData.achievements.map(a => ({...a}));
      let newAchievementUnlocked = null;

      if (score === totalQuestions) {
          const quizWhiz = newAchievements.find(a => a.id === 'quizWhiz');
          if (quizWhiz && !quizWhiz.unlocked) {
              quizWhiz.unlocked = true;
              newAchievementUnlocked = achievementDetails.quizWhiz;
          }
      }
      ...
      try {
        ...
        if (newAchievementUnlocked && appSettings.showNotifications) {
            setNewlyUnlockedAchievement(newAchievementUnlocked);
        }
      } catch (error) {
  ...
  const handleStartSmartReview = async () => {
      ...
      if (snapshot.empty) {
          alert("No questions are due for review right now. Great job! Try a Quick Review instead.");
          return;
      }
      
      const reviewQuestionIds = snapshot.docs.map(doc => doc.id);
      const reviewQuestions = allMockQuestions.filter(q => reviewQuestionIds.includes(q.id));
      
      setSrsQuiz(reviewQuestions);
      handleNavigation('srsQuiz');
  };

  const navItems = [ { name: 'Dashboard', icon: <BarChart2 /> }, { name: 'Progress', icon: <LineChart /> }, { name: 'Study Domains', icon: <BookOpen /> }, { name: 'Practice Quizzes', icon: <Target /> }, { name: 'Leaderboards', icon: <Users /> }, { name: 'Settings', icon: <Settings /> }, ];
  const handleNavigation = (view, domain = null, topic = null) => { setViewState({ view, domain, topic }); };
  const handleBack = () => {
      if (viewState.view === 'simulation') { handleNavigation('topicDetail', viewState.domain, 'sop'); }
      else if (viewState.view === 'memoryGame' || viewState.view === 'srsQuiz') { handleNavigation('PracticeQuizzes'); }
      else if (viewState.view === 'topicDetail') { handleNavigation('domainDetail', viewState.domain); }
      else if (viewState.view === 'domainDetail') { handleNavigation('StudyDomains'); }
  };
  
  const renderView = () => {
    if (!userData || !db) { return <LoadingView />; }
    switch(viewState.view) {
      case 'Dashboard': return <DashboardView user={userData} onAdjustPlan={() => setShowTimelineModal(true)} onNavigate={handleNavigation} onTogglePlanItem={handleTogglePlanItem} />;
      case 'Progress': return <ProgressReportView user={userData} />;
      case 'StudyDomains': return <StudyDomainsView domains={userData.domains} onNavigate={handleNavigation} />;
      case 'domainDetail': return <DomainDetailView domainName={viewState.domain} onNavigate={handleNavigation} onBack={handleBack} />;
      case 'topicDetail': return <TopicDetailView domainName={viewState.domain} topicId={viewState.topic} onNavigate={handleNavigation} onBack={handleBack} />;
      case 'simulation': return <SimulationView simulationId={viewState.topic} onBack={handleBack} />;
      case 'PracticeQuizzes': return <PracticeQuizzesView onQuizComplete={handleQuizCompletion} userId={userId} db={db} onNavigate={handleNavigation} onStartSmartReview={handleStartSmartReview} />;
      case 'srsQuiz': return <QuizInterface questions={srsQuiz} onQuizComplete={handleQuizCompletion} userId={userId} db={db} onAnswer={handleAnswer} />;
      case 'memoryGame': return <MemoryGameView onBack={handleBack} />;
      case 'Leaderboards': return <LeaderboardView db={db} currentUserId={userId} />;
      case 'Settings': return <SettingsView user={userData} onUpdateName={handleUpdateName} onResetAccount={handleResetAccount} appSettings={appSettings} onToggleNotifications={() => setAppSettings(s => ({...s, showNotifications: !s.showNotifications}))} />;
      default: return <PlaceholderView viewName={viewState.view} />;
    }
  };

  return (
    <div className="bg-slate-900 text-slate-200 font-sans min-h-screen flex">
      <style>{`
        :root {
            --color-blue: #3b82f6;
            --color-green: #22c55e;
        ...
        @keyframes pulse-slow {
            50% { box-shadow: 0 0 0 4px rgba(250, 204, 21, 0), 0 0 0 8px rgba(250, 204, 21, 0); }
            100% { box-shadow: 0 0 0 0 rgba(250, 204, 21, 0.4), 0 0 0 4px rgba(250, 204, 21, 0); }
        }
      ...
      `}</style>
      {showTimelineModal && <TimelineModal onGeneratePlan={handleGeneratePlan} domains={userData.domains} />}
      <AchievementNotification achievement={newlyUnlockedAchievement} onclose={() => setNewlyUnlockedAchievement(null)} />
      <LevelUpModal level={levelUpInfo?.level} onclose={() => setLevelUpInfo(null)} />
      
      <nav className="w-20 lg:w-64 bg-slate-900/80 backdrop-blur-md border-r border-slate-800 p-4 lg:p-6 flex flex-col">
        <div className="flex items-center space-x-2 mb-10"> <BrainCircuit className="w-8 h-8 text-blue-500" /> <h1 className="text-xl font-bold text-white hidden lg:block">NBHWC Prep</h1> </div>
        <ul className="space-y-3"> {navItems.map(item => ( <li key={item.name}> <a href="#" onClick={(e) => { e.preventDefault(); handleNavigation(item.name); }} className={`flex items-center space-x-4 p-3 rounded-lg transition-colors duration-200 ${viewState.view === item.name || (item.name === 'Study Domains' && (viewState.view === 'domainDetail' || viewState.view === 'topicDetail' || viewState.view === 'simulation')) || (item.name === 'Practice Quizzes' && (viewState.view === 'memoryGame' || viewState.view === 'srsQuiz')) ? 'bg-blue-600 text-white shadow-lg' : 'hover:bg-slate-800'}`}> {item.icon} <span className="hidden lg:block font-semibold">{item.name}</span> </a> </li> ))} </ul>
        <div className="mt-auto bg-slate-800/50 p-4 rounded-xl border border-slate-700 hidden lg:block"> <h4 className="font-bold text-white">Exam Simulation</h4> <p className="text-sm text-slate-400 mt-1 mb-3">Ready for a full practice test?</p> <button onClick={() => handleNavigation('PracticeQuizzes')} className="w-full bg-green-600 hover:bg-green-500 text-white font-bold py-2 px-4 rounded-lg transition-colors"> Start Now </button> </div>
        <div className="mt-4 pt-4 border-t border-slate-700 text-center hidden lg:block">
            <p className="text-xs text-slate-500">Created by</p>
            <a href="https://www.energizeandempower.com" target="_blank" rel="noopener noreferrer" className="text-sm font-semibold text-slate-400 hover:text-blue-400 transition-colors">Energize & Empower</a>
        </div>
      </nav>
      <main className="flex-1 p-6 sm:p-8 lg:p-10 overflow-y-auto"> {renderView()} </main>
    </div>
  );
}

const TimelineModal = ({ onGeneratePlan, domains }) => {
    const today = new Date().toISOString().split('T')[0];
    const [startDate, setStartDate] = useState(today);
    const [targetDate, setTargetDate] = useState(new Date(new Date().setMonth(new Date().getMonth() + 3)).toISOString().split('T')[0]);
    const [hours, setHours] = useState(10);
    const [isGenerating, setIsGenerating] = useState(false);

    const generatePlan = () => {
        setIsGenerating(true);
        const sortedDomains = [...domains].sort((a, b) => a.progress - b.progress);
        const weakestDomain = sortedDomains[0];
        const secondWeakestDomain = sortedDomains[1];
        const newPlan = [ { task: `Take a quiz on '${weakestDomain.name}'`, time: 20, completed: false }, { task: `Review a topic in '${secondWeakestDomain.name}'`, time: 30, completed: false }, { task: `Take a quiz on '${secondWeakestDomain.name}'`, time: 20, completed: false }, { task: `Review a topic in '${weakestDomain.name}'`, time: 30, completed: false } ];
    ...
    return (
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center z-50 p-4">
            <div className="bg-slate-800 border border-slate-700 rounded-2xl p-8 max-w-lg w-full shadow-2xl">
                <h2 className="text-2xl font-bold text-white mb-2">Create Your Personalized Study Plan</h2>
                <p className="text-slate-400 mb-6">Let's tailor a timeline to fit your schedule and goals.</p>
                <div className="space-y-4">
                    <div> <label className="text-sm font-medium text-slate-300 block mb-2">Study Start Date</label> <input type="date" value={startDate} onChange={e => setStartDate(e.target.value)} className="w-full bg-slate-700 border-slate-600 rounded-lg p-2 text-white focus:ring-blue-500 focus:border-blue-500" /> </div>
                    <div> <label className="text-sm font-medium text-slate-300 block mb-2">Target Confidence Date</label> <input type="date" value={targetDate} onChange={e => setTargetDate(e.target.value)} className="w-full bg-slate-700 border-slate-600 rounded-lg p-2 text-white focus:ring-blue-500 focus:border-blue-500" /> </div>
                    <div> <label className="text-sm font-medium text-slate-300 block mb-2">Available Study Hours Per Week</label> <input type="number" value={hours} onChange={e => setHours(e.target.value)} className="w-full bg-slate-700 border-slate-600 rounded-lg p-2 text-white focus:ring-blue-500 focus:border-blue-500" /> </div>
                </div>
                <div className="flex justify-end space-x-4 mt-8">
                    <button onClick={() => onGeneratePlan(null)} className="px-6 py-2 rounded-lg text-white bg-slate-600 hover:bg-slate-500 transition-colors">Cancel</button>
                    <button onClick={generatePlan} disabled={isGenerating} className="px-6 py-2 rounded-lg text-white bg-blue-600 hover:bg-blue-500 font-semibold transition-colors flex items-center disabled:bg-slate-500">
